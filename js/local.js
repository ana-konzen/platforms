...
export function setup() {
  playerData.player1.boundaries = {
    left: STYLE.wallW / 2,
    right: width / 2 - STYLE.wallW / 2,
  };
  playerData.player2.boundaries = {
    left: width / 2 + STYLE.wallW / 2,
    right: width - STYLE.wallW / 2,
  };
  ...
  for (const playerKey in playerData) {
    ...
    if (partyIsHost()) {
      shared[playerKey] = {
        ...
        target: { x: randomPos(player.boundaries), y: height - STYLE.targetH / 2 - 10 },
        color: player.color,
      };
    }
    player.ball = Bodies.circle(shared[playerKey].ball.x, 0, STYLE.ballRadius, ballOptions);
